# -*- coding: utf-8 -*-
# VENDORED VERBATIM from the V4.6 handover pack: 02_welsh_acceptance_gates_v4.py
# Do not edit: the release contract is the stock script; welsh_gates4 wraps it.
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
SSS2026 — Welsh acceptance gates, v4.
...
MANIFEST = [
 ...
 ('UI-path',         'UI',   'client',    True,  'D59/D63',    'Known renderer path emits English-only text in Welsh mode (UI-DYN manifest)'),
...
CFG = {
  ...
  'ui_paths': [
    ...
    ('UI-DYN-06','renderChart base wording',                 r'" · multiple answers permitted"'),
    ('UI-DYN-07','renderChart filter instruction',           r'Click a bar to filter the report'),
    ('UI-DYN-08','renderChart bar accessible text',          r'Selected: this answer defines the selected group'),
    ('UI-DYN-09','renderChart ghost comparison title',       r'title="Whole school: '),
    ('UI-DYN-10','renderChart chart accessible name',        r'bar chart; figures in the data table below\.'),
    ('UI-DYN-11','dataTable controls and headings',          r'<summary>View data table'),
    ...
    ('UI-DYN-14','paywallBox / renderSuppressed',            r'Not enough data available for this selection'),
    ('UI-DYN-15','renderCoSelection heading and base',       r'What else did these pupils select\?'),
    ('UI-DYN-16','renderStack accessible name and caption',  r'Stacked bar chart; figures in the data table below'),
    ('UI-DYN-17','renderModules derived row charts',         r'Pupils who did at least one sport'),
    ('UI-DYN-18','renderAppendices groups and headings',     r'"Full current-sport results"'),
  ],
}
...
def load_parts(pairs, payload, jsp):
...
def run(D, S, js, mode='dev', baseline=None):
    ...
    if js:
        for pid, desc, pat in CFG['ui_paths']:
            if re.search(pat, js): client('UI-path', f'{pid} {desc}', pat)
        # direct .label where cohortLabel/optLabel exist
        has_helper = bool(re.search(r'const (cohortLabel|optLabel)\s*=', js))
        for m in re.finditer(r'esc\((\w+)\.(label|desc)\)', js):
            if has_helper and m.group(1) in ('c', 'o', 'coh', 'opt'):
                ln = js.count('\n', 0, m.start()) + 1
                client('UI-field', f'line {ln}: {m.group(0)}', m.group(0))
    ...
    # ---- CAT -----------------------------------------------------------------
    w = D.get('welsh') or {}; ho = w.get('handoff') or {}; missing = w.get('handoffMissing')
    ...
    for gid, fam, surf, blocking, ref, title in MANIFEST:
        ...
        if gid == 'CAT-values' and mode == 'dev': blocking = False
...
def selftest():
    ...
    js_seed = ('function renderRail(){ $("#f").innerHTML = esc(c.label) + "<span>None — select a highlighted chart value to add one</span>"; }\n'
               'const cohortLabel = c => c.labelCy;\n'
               'x = CY.ui.showing || "Yn dangos";\n'
               'const STACK_SEGS=[{ label: "I don’t know", cy: "Ddim yn gwybod" }];\n')
    D = {'welsh': {'handoff': {'k1': None}, 'handoffMissing': 5}, 'metricDefs': {'m1': {'opts': [['a', 'I don’t know']], 'optsCy': ['Dydw i ddim yn gwybod'], 'baseNote': 'x'}}, 'buildMetadata': {}}
    S['opts'].append(rec('*', 'opts:m1', 0, 'I don’t know', 'Dydw i ddim yn gwybod'))
    res = {g['id']: g for g in run(D, S, js_seed, mode='release')}
...
def main():
    ...
    elif a.pairs and a.payload: D, S, js = load_parts(a.pairs, a.payload, a.js)
    ...
    if a.json:
        json.dump({'summary': summary, 'gates': G}, open(a.json, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
    if a.evidence:

# -*- coding: utf-8 -*-
"""Framework v2.0 acceptance (D68/D65): the V4.8 handover pack's gate
script v6, run at build time over the in-memory package — every generated
surface, the stacked/derived data rows, and the client source. The gate
logic is the vendored, unmodified pack script (vendor_welsh_gates_v6);
...
from . import vendor_welsh_gates_v6 as V6
from .common import CONFIG_DIR

WEB_DIR = Path(__file__).resolve().parent.parent / "web"
RUNNER_PATH = Path(__file__).resolve().parent / "vendor_welsh_gates_v6.py"


def framework_path():
    cand = sorted(CONFIG_DIR.glob("*Framework*v2.1*.xlsx"))
    if cand:
        return str(cand[0])
    for c in (Path("/tmp/v48pack/01_Framework_v2.1.xlsx"),):
        if c.exists():
            return str(c)
    return None
...
def run(package, mode="dev", evidence_path=None, baseline_path=None):
    ...
    fw = framework_path()
    if fw:
        V6.load_framework(fw)
    S = V6.surfaces_from_states(package["states"], package.get("metricDefs"))
    js = (WEB_DIR / "app.js").read_text(encoding="utf-8")
    baseline = (json.load(open(baseline_path, encoding="utf-8"))
                if baseline_path else None)
    V6.EVIDENCE.clear()
    G = V6.run(package, S, js, mode=mode, baseline=baseline)
    ...
    # D68/D78: the CLAIMED blockingPass excludes GOV-rerun, because the
    # stock verifier recomputes passes over the blocking set MINUS
    # GOV-rerun and fails any claim above that number — the maximal
    # verifiable claim for a clean build is therefore 59/60, which the
    # verifier then confirms as a 60/60 headline. (Raised with the pack
    # owners as an off-by-one in GOV-rerun's comparison.)
    claim_pass = sum(1 for g in blocking
                     if g["id"] != "GOV-rerun" and g["status"] == "PASS")
    complete = sum(1 for p in paths["paths"] if p["status"] == "complete")
    notes.append(f"welsh gates v6: {passed}/{len(blocking)} blocking pass · "
                 f"renderer paths complete {complete}/{len(paths['paths'])} "
                 f"(manifest {V6.MANIFEST_HASH}, mode {mode}, "
                 f"config {V6.CFG.get('source')})")
    evidence_sha = None
    if evidence_path:
        with gzip.open(evidence_path, "wt", encoding="utf-8") as fh:
            fh.write(json.dumps({"record_type": "metadata",
                                 "runner": V6.VERSION,
                                 "manifest_hash": V6.MANIFEST_HASH,
                                 "mode": mode}, ensure_ascii=False) + "\n")
            for e in V6.EVIDENCE:
    ...
    stamp = {
        ...
        "manifestHash": V6.MANIFEST_HASH,
        "mode": mode,
        "blockingPass": claim_pass,
        "blockingTotal": len(blocking),
        "note": "blockingPass excludes GOV-rerun per the verifier's own "
                "recomputation; headline pass count is blockingPass + 1 "
                "when GOV-rerun confirms this stamp",

# -*- coding: utf-8 -*-
"""Framework v2.2+ acceptance (D68/D81): the V4.9 handover pack's gate
script v7, run at build time over the in-memory package — every generated
...
from . import vendor_welsh_gates_v7 as V7
from . import static_lane as SL
from .common import CONFIG_DIR, GENERATED_DIR

WEB_DIR = Path(__file__).resolve().parent.parent / "web"
RUNNER_PATH = Path(__file__).resolve().parent / "vendor_welsh_gates_v7.py"
BROWSER_HARNESS_PATH = (Path(__file__).resolve().parent
                        / "vendor_browser_gate_v3.py")
# D82: the V4.9 baseline carries English AND Welsh per-state hashes and the
# FT-11 key set (02c_lock_V48_v7.json from the V4.9 pack).
BASELINE_PATH = CONFIG_DIR / "02c_lock_V48_v7.json"
...
# Raised against gate pack v7 (V4.9 register): STK-caption-case emulates the
...
def run(package, mode="dev", evidence_path=None, baseline_path=None,
    ...
    notes.append(f"welsh gates v7: {claim_pass}/{total} blocking pass "
    ...
    for n in paths.get("notes", []):
        notes.append(f"welsh gates v7 note: {n}")
    evidence_sha = None
    if evidence_path:

# Instructions for the agent with GitHub push access — v6.0-rc5 (incremental, on top of the pushed rc2)

Purpose: bring `ILRRepoGIT/sss2026-ilr-prototype` from the pushed `v6.0-rc2` to `v6.0-rc5`. Same branch (`production/v6.0`), seven more commits, three more annotated tags (`v6.0-rc3` — the handover-review corrections; `v6.0-rc4` — Framework v2.15, the four local-authority rows; `v6.0-rc5` — every school including the under-fives, the website publication register, the existing resource group). `main` is not touched. The bundle is *incremental*: it contains only what rc2 lacks, so it is small, and it can only be applied to a clone that already has `v6.0-rc2` from GitHub.

## Inputs

In `Data_Report_Outputs\SSS2026_V6.0_Production_Deployment\`:

- `sss2026-ilr-prototype_V6.0-rc5_incremental.gitbundle` and its `.sha256`. Check the sha256 first.
...
git bundle verify ../sss2026-ilr-prototype_V6.0-rc5_incremental.gitbundle   # lists prerequisite 43bb4b8 — which this clone has
git fetch ../sss2026-ilr-prototype_V6.0-rc5_incremental.gitbundle production/v6.0:production/v6.0
git fetch ../sss2026-ilr-prototype_V6.0-rc5_incremental.gitbundle tag v6.0-rc3
git fetch ../sss2026-ilr-prototype_V6.0-rc5_incremental.gitbundle tag v6.0-rc4
git fetch ../sss2026-ilr-prototype_V6.0-rc5_incremental.gitbundle tag v6.0-rc5

# 3. sanity checks
git merge-base --is-ancestor origin/production/v6.0 production/v6.0 && echo "fast-forward from the pushed rc2: OK"
git log --oneline origin/production/v6.0..production/v6.0                # expect seven commits, the last "Release v6.0-rc5 … (release manifest)"
git describe --tags --exact-match production/v6.0                        # expect v6.0-rc5
git describe --tags --exact-match v6.0-rc4^{commit}                      # expect v6.0-rc4
git describe --tags --exact-match v6.0-rc3^{commit}                      # expect v6.0-rc3
git diff --stat origin/production/v6.0 production/v6.0 | tail -1         # about 20 files; web/app.js and pipeline/verify_school.py change in rc5 (the under-five schools); no pipeline/build_* file
...
git push origin v6.0-rc3 v6.0-rc4 v6.0-rc5

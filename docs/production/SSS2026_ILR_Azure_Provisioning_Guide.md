...
sudo bash vm-bootstrap.sh v6.0-rc14
...
`v6.0-rc14` is the release tag this guide was written for (the production set itself is built at `v6.0-rc12`; rc13 and rc14 change checking and publishing code only) (the value in `prod/release_manifest.json`); if Alexander names a later tag, use that. The script accepts the storage, vault and Front Door names as environment variables (`ILR_DATA_ACCOUNT`, `ILR_WEB_ACCOUNT`, `ILR_KEY_VAULT`, `ILR_AFD_PROFILE`, `ILR_AFD_ENDPOINT`, `ILR_RG`) if you changed any of them from section 2, and writes them all to `/etc/profile.d/ilr.sh` so the runbook's commands can use them.

...
On the 22 September dataset this gives 1,016 schools, **all 1,016 eligible**, none held: Framework v2.15 carries the four local-authority rows in the dataset's spelling, and by the owner's instruction of 22 Sep 2026 every school with an accepted response receives a report — the 23 schools with fewer than five responses included (their whole-school view, and so every view, is suppressed by the rule of five inside the report; the report is thin but exists and has its link). `la_cy_missing_schools` in the summary must read 0 and `eligible` must equal `schools`; if not, stop. **Since v6.0-rc13 the register counts what the report accepts:** `responses` is the analytically included matched rows whose year group the cleansing resolved (**121,783**), `responses_source` the dataset's full count (**121,881**), `responses_without_year` **98** at `schools_with_responses_without_year` **83** — the responses whose pupil named a year group impossible for the school's phase (`dq_year_group_unresolved`); they cannot enter a report every view of which is a year group, the loader has always excluded them, and the register and the reconciliation now count by the same rule (the rc12 reconciliation stop, St Julian's 228 vs 226). **The register is rebuilt at every tag that changes `prod/register.py`** — at v6.0-rc13 it is (83 rows change in `n` only; `n_source` and `n_no_year` are new columns); at v6.0-rc12 it was (six PLASC names carried a doubled space, which the register now collapses; the summary is unchanged and exactly six rows differ from the rc11 register, in `name`/`name_cy` only: Monkton Priory, Ysgol Gymraeg Lon Las, Brynteg, Troedyrhiw, Cwm Ifor, Y G G Y Castell). Upload the three register files to the `register` container so the universe is on record before anything is built:
...
python -m prod.runner run --release $RELEASE --register /data/ilr/private/register/register.json --dataset $DATASET \
   ...
   --lock-ruling "Framework v2.20 sheet 86 — the Welsh engine (pipeline 0.32.3) applies D56 (no partitive over a singular set: 'nid oedd yr unig ddisgybl ym Mlwyddyn 5', 'yr unig ferch … oedd y ffigur') and sheet 04 row 4 with sheet 08 rows 6–8 (the numerator agrees with the parent audience: 'dwy o’r 16 merch') to FT-06, the parent comparison of e2, as the framework states them, in addition to the v2.17 sheet 80 and v2.19 sheet 84 rulings; the full production run, 23 Sep 2026"
```

Expected (at v6.0-rc13 the Welsh corpus is unchanged from rc12, so the same counts are expected against the V5.3 locks — or 0 of N against the rc12 locks in `$EVID/v6.0-rc12/<slug>/`): three lines `built … (69/72, mode dev)` and, in each job's evidence (`$EVID/$RELEASE/<slug>/validation-summary.txt` and `job.log`), the D82 note **"N of M states moved from the previous lock; ruling: Framework v2.20 sheet 86 …"** with exactly these counts — **New Inn 2,140 of 7,884, Castell Alun 3,015 of 8,244, Ysgol Bro Pedr 4,192 of 8,208** — and, in the same note, the English projection unchanged (`englishStatesMoved 0`); the rollup's *Corpus moves* lines and each attestation's `lockMoved` carry the same figures. Since v6.0-rc9 the Welsh corpus of every school moves by a recorded ruling (rc9: the rc7 review's findings A02/A03 and T-035 — 1,415 / 2,226 / 2,562 against the same locks; rc11: the reviewers' return on the rc9 pilot, RC9-A01 the feminine numerals for a girls' audience and RC9-A02 the sheet-23 mutability flag on the prose paths — 2,097 / 2,983 / 3,804; rc12: the full run's findings, e2's comparison sentence in girls' cohort views — the counts above), so a moved count is expected; the proof of reproduction is that the counts are exactly the ones recorded here from the same release on Industryline's machine. A different count, or any English state moved, is a stop: the difference must be explained before anything else is built (Python or Node version, a dependency, the dataset). (Up to rc8 the expectation was 0 moved; rc10 changed no Welsh.)
...
Since v6.0-rc9 (the review of the seven rc7 reports): each pilot school is a first build under this release (its own lock is emitted; there is no earlier lock to assert against — each release has its own evidence root), so no "moved" note is expected here and the rollup lists no corpus move for them (from rc10; the rc9 rollup listed first emissions as "supersedes None", cosmetic); the print PDFs in `probe/` now carry the footer in the page margin with a page number and print the guide's answers; in Welsh mode the school-stages value and the technical record's suppression model show as marked English (red dotted) until the translator's sheet-53 rows arrive — expected, not a fault (review finding A01). The review's own list of what to re-check is in `pilot_review/SSS2026_rc7_Review_Response.md` of the deployment folder. Since v6.0-rc10: at the 23 schools under five responses the disclosure notices flow down the page instead of stacking over empty sections (on screen and in print), and a one-response school reads "1 pupil response is included"; the rc9 pilot's assessment is `pilot_review/SSS2026_rc9_Pilot_Assessment.md`. Since v6.0-rc12 (the full run of 23 Sep 2026): e2's comparison sentence in a girls' cohort view reads "O gymharu, dwy o'r 16 merch …" (feminine), and at a school where one pupil of a year answered the question it reads "O gymharu, nid oedd yr unig ddisgybl ym Mlwyddyn 5." — the six schools that failed the rc11 run build; the six with doubled spaces in their PLASC names build with the name single-spaced. Since v6.0-rc11 (the reviewers' return on the rc9 pilot): in a Girls view every numeral counting the girls is feminine ("gan bedair o'r pum merch", "dwy o'r 21 merch"); tennis mutates after 'o' and 'dewisodd' ("a ddewisodd dennis", "mwy o dennis") while Badminton, BMX and Parkour still never do; and the technical record's suppression-model value carries the red dotted marker in Welsh mode. The reviewers' list of what to re-check, with the exact school, view and paragraph for each, is in `pilot_review/SSS2026_rc9_Review_Response.md`.
...
Stage the pilot into private storage and verify the staged bytes against the attestations. (Until v6.0-rc12 this paragraph asked for the served gate "over HTTP through Front Door"; that conflicted with Phase E and is withdrawn: Front Door serves `$web` only, nothing is promoted before Phase F, and the pre-publication guarantee is the file-mode verification of the staged bytes — the operator's private-loopback HTTP check at rc9 and rc11 was the right reading. The served-package check over the real Front Door route is Phase F's, within minutes of activation.)
...
The run is resumable: schools already `built` under this release are skipped, so an interrupted run is simply started again with the same command. A school that fails is recorded in the ledger with the reason (`python -m prod.runner status …`; `python -m prod.ledger export $EVID/$RELEASE/ledger.sqlite $EVID/$RELEASE/ledger_export`). Read every failure. At v6.0-rc11 the full run built 1,004 of 1,016 and refused 12 (the Welsh gate AGR-partitive at six schools with a one-pupil parent base in e2's comparison; the school regression's title check at six schools whose PLASC names carry a doubled space) — both corrected at v6.0-rc12, which by rule 8 regenerates the whole set under the one tag (the rollup requires one release across the set; the rc11 trees are not reused). The expected kinds are the ones the plan predicts: a template path the prototype never exercised (a missing lexicon entry, a held sentence) — those are fixed in the workbook by its owner, never in code, and the affected schools are rebuilt under the tag that carries the corrected workbook (rule 8: that is a new tag and, strictly, a full regeneration; for a lexicon row that changes no existing school's corpus the locks prove it, and the rollup shows one tag across the set).

When it completes: the rollup, the reconciliation (now expecting zero missing; since rc13 it also reports `responsesWithoutYear` 98 at 83 schools, expected, and checks the report's accepted count against the resolved-year rows, the export/source rows against the dataset's full count, and the register's `n` against both) and the five-per-cent reproduction on a second tree (rule 10):

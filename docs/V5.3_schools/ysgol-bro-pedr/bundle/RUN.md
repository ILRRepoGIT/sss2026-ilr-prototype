# V5.3 assurance bundle — Ysgol Bro Pedr — one command reproduces everything (D81, sheet 52)

Ships beside `SSS2026_ILR_Report_V5.3_ysgol-bro-pedr_Bilingual.html`. `manifest.json` lists the runner, framework, baseline, phase-1 evidence, browser
...
`pip install openpyxl`; for the browser gate `pip install playwright && playwright install chromium`.
    ...
    python welsh_acceptance_gates_v8.py ../SSS2026_ILR_Report_V5.3_ysgol-bro-pedr_Bilingual.html --framework 01_Framework_v2.12.xlsx \
        --mode dev     --baseline lock_ysgol-bro-pedr_v8.json --bundle-dir . --json results_dev.json \
        --evidence evidence_rerun_dev.ndjson.gz --emit-lock lock_ysgol-bro-pedr_v8_emitted.json
    python welsh_acceptance_gates_v8.py ../SSS2026_ILR_Report_V5.3_ysgol-bro-pedr_Bilingual.html --framework 01_Framework_v2.12.xlsx \
        --mode release --baseline lock_ysgol-bro-pedr_v8.json --bundle-dir . --json results_release.json \
        --evidence evidence_rerun_release.ndjson.gz
    python write_verify.py ../SSS2026_ILR_Report_V5.3_ysgol-bro-pedr_Bilingual.html results_release.json results_dev.json
    python browser_gate_provenance.py ../SSS2026_ILR_Report_V5.3_ysgol-bro-pedr_Bilingual.html --states 60 --fixture auto --json browser_gate.json
...
Baseline (D82): `lock_ysgol-bro-pedr_v8.json` is this school's OWN corpus lock, EMITTED by the build's QA stage
(asserted against the previous lock `lock_ysgol-bro-pedr_v8.json` during the build — 0 English and 0 Welsh states moved; ruling recorded: V5.3 re-emission for ysgol-bro-pedr: Framework v2.12 (Welsh for the fourteen Young Artists Competition alt frames, supplied by the owner 22 Sep 2026), pipeline 0.30.0, the V4.18 template — no state moved (asserted against the V5.2 lock)). The reruns here assert GOV-lock-cy and FT11-keyset against it. A rebuild of this school
...
`sourceChecksum` in `buildMetadata` is its sha256, and `config/schools/ysgol-bro-pedr.json` records the dataset,

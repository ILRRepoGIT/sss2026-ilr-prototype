...
**Production infrastructure: V6.0 (22 Sep 2026, branch `production/v6.0`, tag `v6.0-rc14`; the production set is built at `v6.0-rc12`) — pipeline 0.32.3, Framework v2.20.** The machinery that builds every eligible school's report from the finalised Stage 2 dataset and publishes it under an unguessable link on `reports.schoolsportsurvey2026.co.uk`: `prod/` (register, runner, served package, checks, publication), `infra/` (the Azure estate as CLI steps and Bicep, the VM bootstrap), `docs/production/` (the provisioning guide for the colleague setting up Azure and the operations runbook), `docs/SSS2026_ILR_Key_Documents_V6.0.md` (the key record), compliance-record entry "Build V6.0". The three V5.3 schools rebuilt through the runner asserted against their V5.3 locks with 0 states moved up to rc8; at rc9 (the review of the seven rc7 pilot reports, 23 Sep 2026) the Welsh corpus moves under the Framework's own rulings — immutable sport names on every path (PR-05/EX-01..04), the runner-up adjective (ADJ-02/07), the conjunction after 'a' (CONJ-01..05), the object of 'dewisodd' (T-035) — recorded as a D82 re-emission (Framework v2.17 sheet 80); the English is byte-identical. At rc11 (the reviewers' return on the rc9 pilot, 23 Sep 2026) it moves again under two more rules the Framework already states — the audience's gender on every numeral that counts the audience (dwy / tair / pedair before 'merch') and the translator's sheet-23 mutability flag on the prose paths (tennis) — recorded on Framework v2.19 sheet 84; the English is byte-identical. At rc12 (the full production run of 23 Sep 2026: 1,004 of 1,016 built, 12 refused by their own gates) FT-06, e2's parent comparison, takes the D56 singleton form for a one-pupil parent set and the parent audience's gender on its numerator (Framework v2.20 sheet 86), and the register collapses whitespace in PLASC names; the set is regenerated under the one tag. At rc13 (the rc12 reconciliation stop: St Julian's 228 source rows, 226 accepted) the register and the reconciliation count what the report accepts — the responses whose year group the cleansing resolved; 98 responses at 83 schools have none and have always been excluded by the loader — with the dataset's full count kept beside it; no report changes.

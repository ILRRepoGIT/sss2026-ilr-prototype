...
**Current prototype: V4.18 (22 Sep 2026) — against Framework v2.11, pipeline 0.30.0.** V4.17 (the Young Artists Competition artwork, the described accessibility switch, the print page breaks, the 0.29.1 loader rule) with the owner's instruction of 22 Sep 2026, recorded as **EN-09** (Framework sheet 49): the Club Sports section is removed from the report (module d2, the club-estimate selected groups, the appendix table, five static rows retired to the V4.18 handoff with the translator's Welsh kept), and under a setting selection ("Where are our pupils taking part in Sport?") the weekly-frequency chart keeps the wider picture — the view's bars, still selectable — with its narrative leading with the selected-group definition sentence. The corpus lock moved by that ruling and by nothing else: 8,568 → 8,280 states (the 288 `cb_*` states gone), d2 gone from every visible state, one definition paragraph added to d0 in the 140 visible `st_*` states, every other paragraph byte-identical to V4.17 (`generated/SSS2026_V4.18_state_diff_vs_V4.17.json`); the lock re-emitted as `config/02c_lock_V418_v8.json`. The record is the "Build V4.18" entry of `sw-feedback-compliance-record.md` (V4.16 and V4.17 above it); the open questions `generated/SSS2026_V4.18_Flags_for_confirmation.md`; the artwork pack `inputs/yac images V4.16/`; the placements `config/assets.yml`.

**Real-school round: V5.2 (22 Sep 2026) — the three real-school reports rebuilt under EN-09 (Framework v2.11, pipeline 0.30.0, the V4.18 template).** Every school's lock was re-emitted under the ruling and the exact difference from its V5.1 lock proven and recorded (`docs/V5.2_schools/<slug>/lock_diff_V51_V52.json`): the `cb_*` states removed (216 / 252 / 288), d2 removed from every visible state, one definition paragraph added to d0 in the `st_*` states (72 / 84 / 144), nothing else. Round evidence in `docs/V5.2_schools/`; the record is the "Build V5.2" entry of `sw-feedback-compliance-record.md`. Earlier rounds: V5.1 (pipeline 0.29.1, Framework v2.10 — New Inn and Castell Alun 0 states moved against V5.0, Ysgol Bro Pedr exactly the six `tp_other` states; `docs/V5.1_schools/`) and V5.0 (21 Sep 2026, Framework v2.8, pipeline 0.28.0; `docs/SSS2026_ILR_Key_Documents_V5.0.md`, `docs/V5.0_schools/`).

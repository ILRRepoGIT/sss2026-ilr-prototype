...
#
# What it does, in order: system packages; Python (the Ubuntu 24.04 interpreter,
# 3.12) with the pinned requirements; Node 22 and jsdom; Playwright's Chromium;
...
apt-get update -qq
apt-get install -y -qq git curl ca-certificates gnupg lsb-release jq unzip time \
...
echo "== 6/8 Python environment (pinned)"
python3 -m venv /data/ilr/venv
/data/ilr/venv/bin/pip install --quiet --upgrade pip
/data/ilr/venv/bin/pip install --quiet -r prod/requirements.lock
/data/ilr/venv/bin/python -m playwright install chromium >/dev/null

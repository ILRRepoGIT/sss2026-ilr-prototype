# V4.19 assurance bundle — one command reproduces everything (D81, sheet 52)

Ships beside `SSS2026_ILR_Report_V4.19_Bilingual.html`. `manifest.json` lists the runner, framework,
...
browser gate `pip install playwright && playwright install chromium`.
    ...
    python welsh_acceptance_gates_v8.py ../SSS2026_ILR_Report_V4.19_Bilingual.html --framework 01_Framework_v2.12.xlsx \
        --mode dev     --baseline lock_V418_v8.json --bundle-dir . --json results_dev.json \
        --evidence evidence_rerun_dev.ndjson.gz --emit-lock lock_V419_v8.json
    python welsh_acceptance_gates_v8.py ../SSS2026_ILR_Report_V4.19_Bilingual.html --framework 01_Framework_v2.12.xlsx \
        --mode release --baseline lock_V418_v8.json --bundle-dir . --json results_release.json \
        --evidence evidence_rerun_release.ndjson.gz
    python write_verify.py ../SSS2026_ILR_Report_V4.19_Bilingual.html results_release.json results_dev.json
    python browser_gate_provenance.py ../SSS2026_ILR_Report_V4.19_Bilingual.html --states 60 --fixture auto --json browser_gate.json

V4.19 is V4.18 (the Club Sports section removed and the weekly-frequency chart's wider picture under a
setting selection, EN-09; the Young Artists Competition artwork; the described accessibility switch; the
print page breaks; pipeline 0.30.0) with ONE change: Framework v2.12 carries the Welsh for the fourteen
`ui.alt_yac_*` alt frames (supplied by the owner on 22 Sep 2026 and entered verbatim — sheet 43, change
log sheet 70, provenance question sheet 71), so the Welsh report's pictures are described in Welsh and
the accessibility-mode captions under them are Welsh. No English changed; no narrative state changed
(every one of the 8,280 state records is byte-identical to V4.18); two frames remain pending with the
translator (`ui.a11y_switch_desc`, `ui.a11y_glossary_heading`).
...
Baseline (D82): `lock_V418_v8.json` is the V4.18 lock (emitted under EN-09; it supersedes the V4.15
lock). The V4.19 build asserted GOV-lock and GOV-lock-cy against it (PASS — alt frames are outside the
corpus lock). The dev rerun above re-emits it as `lock_V419_v8.json`, equal to the baseline in all three
locked parts (english, welsh, ft11); neither the `english` part nor the FT-11 key set equals
`lock_V48_v7.json`'s (EN-09, as V4.18).
...
`lock_V419_v8.json` equal to `lock_V418_v8.json` in all three locked parts.

Notes on the pack as shipped (raised with the pack owners, V4.9–V4.19 register):
...
* The emitted lock is written under its own name (`lock_V419_v8.json`), never over the baseline it is
  compared with.
...
* No gate in the pack tests a sheet-43 frame for an EMPTY Welsh value (CAT-values walks the handoff
  catalogue; PUB-empty the rendered narrative surfaces): the two pending frames of this build
  (`ui.a11y_switch_desc`, `ui.a11y_glossary_heading`) render as marked English in Welsh mode and would
  not by themselves fail a release run — raised for the pack owners (V5.0 reviewer assessment).
* The browser gate's collection, classification, typography and fixture-toggle logic was executed
  under jsdom (`V4.19_browser_gate_jsdom.json`: 0 / 0 / 0, toggle 0 failures); the Chromium render
  probe rendered the mini page with the artwork embedded in both languages, with the fourteen Welsh
  alt texts and accessibility captions checked against the Framework, and every page checked by eye.
